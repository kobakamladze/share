...
describe("Dynamic Controls Test", function () {
  it.only("Check state of Dynamic control", async function () {
    ...
    assert.isTrue(
      await DynamicControlsPage.isInputFieldEnabled(),
      "Input field on Dynamic Controls Page does not enabled"
    );

    const randomString = crypto.randomUUID();
    await DynamicControlsPage.inputTextIntoInputField(randomString);
    assert.strictEqual(
      await DynamicControlsPage.getText(),
      randomString,
      "Wrong text into input field"
    );
  });
});
