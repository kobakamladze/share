...
export default class BaseElement {
  ...
  getLocator = () =>
    typeof this.locator === "string" ? this.locator : this.locator.selector;

  log = () =>
    `${this.type} - "${this.name}" - by locator "${this.getLocator()}":\n\t`;
  ...
  async findChild(childType, childSelector, childName, options = {}) {
    Logger.info(
      `${this.log()}Find child element "${childName}" by locator: "${childSelector}"`
    );
  ...
  async findAll(childType, childSelector, childName, options = {}) {
    Logger.info(
      `${this.log()}Find all child elements named "${childName}" by locator: "${childSelector}"`
    );
  ...
  async dragAndDropToPosition(target) {
    Logger.info(
      `${this.log()}Drag and drop element to position {x: ${target.x}, y:${
        target.y
      }}`
    );
    await this.state().waitForExist();
  ...
  async dragAndDropToElement(targetElement) {
    Logger.info(
      `${this.log()}Drag and drop element to another element "${
        targetElement.name
      }"`
    );
    await this.state().waitForExist();
